...
class standings:
    ...
    def showstandings_during(self, tournament_type):
		if tournament_type == 'rr':
			for key, value in user_prof_rr.items():
				print "{first}-----{Second} \n".format(first=key, second=value[1])
		else:
			for key, value in user_prof_em.items(): #discuss about this approach.
				if value[2] > 0:
	...
	def showstandings_after(self, tournament_type):
		if tournament_type == 'rr':
			...
			sorted_keys  = sorted(user_prof_rr, key=user_prof_rr.__getitem__)
			sorted_values= sorted(user_prof_rr.value[2])
			for i in range(1, 4):
